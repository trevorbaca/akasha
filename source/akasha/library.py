...
def make_accelerando_rhythm(
    ...
    if force_rest_lts is not None:
        ...
        leaves = abjad.sequence.flatten(lts)
...
def make_cello_solo_rhythm(time_signatures, *, rotation=None):
    ...
    durations = abjad.duration.durations(time_signatures)
    tuplets = rmakers.make_talea_tuplets(durations, counts, 16, tag=tag)
    voice = rmakers.wrap_in_time_signature_staff(tuplets, time_signatures)
    leaf_lists = [_[:] for _ in tuplets]
...
def make_glissando_rhythm(time_signatures):
    ...
    durations = abjad.duration.durations(time_signatures)
    durations = [sum(durations)]
    tuplets = rmakers.make_tuplets(durations, [(8, 1)], tag=tag)
    leaf_lists = [_[:] for _ in tuplets]
    rmakers.attach_beams_to_runs_by_leaf_list(leaf_lists)
    return tuplets
...
def make_growth_rhythm(
    ...
    for i, duration_list in enumerate(duration_lists):
        if i in (1, 3, 5):
            ...
            tuplets = rmakers.make_accelerandi(
                [duration],
                baca.rhythm.interpolations([(1, 2), (1, 8), (1, 16)]),
                tag=tag,
            )
            time_signatures = [abjad.TimeSignature(duration.pair())]
            voice = rmakers.wrap_in_time_signature_staff(tuplets, time_signatures)
            lts = abjad.select.get(baca.select.lts(voice), pattern)
